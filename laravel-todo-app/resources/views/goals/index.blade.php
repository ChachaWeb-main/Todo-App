...
@section('content')
    <div class="container h-100">
        ...
        <!-- 目標の追加用モーダル (Bootstarpで実装)・部品化したモーダル部分を呼び出す (add_goal.blade.php) -->
            ...
            @foreach ($goals as $goal)
                ...
                <div class="col">
                    <div class="card bg-light">
                        ...
                        {{-- Todo --}}
                        {{-- orderBy()メソッドをつなげて並び替える場合はtodos()メソッドにアクセスし、最後にget()メソッドをつなげる --}}
                                ...
                                <div class="card-body">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <h5 class="card-title ms-1 mb-0">
                                            @if ($todo->done)
                                                <s
                                                    style="text-decoration: line-through 4px solid #ff0000;">{{ $todo->content }}</s>
                                            @else
                                                {{ $todo->content }}
                                            @endif
                                        </h5>
                                    ...
                                    <div class="d-flex flex-wrap mx-1 mb-1">
                                        @foreach ($todo->tags()->orderBy('id', 'asc')->get() as $tag)
                                            <span class="badge bg-primary mt-2 me-2 fw-light">{{ $tag->name }}</span>
                                        @endforeach
                                    </div>
                                </div>
                            </div>
